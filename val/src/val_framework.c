...
void val_ep_info_relayer_sync(void)
{
    ...
    LOG(DBG, "Partition Descriptor count: %d", (int)payload.arg2);
    for (i = 0; i < desc_count; i++) {
        ffa_partition_info_t *part = &ret_info[i];

        LOG(DBG, "+---------------- Partition Descriptor [%d] ----------------+", i);
        LOG(DBG, "| ID            : 0x%08x                               |", part->id);
        LOG(DBG, "| Exec Contexts : %-36u     |", part->exec_context);
        LOG(DBG, "| Properties    : 0x%08x                               |", part->properties);
    ...
    for (i = 1; i < ep_count; i++)
    {
        ...
        for (j = 0; j < desc_count; j++)
        {
            if (ep_info[i].id == ret_info[j].id)
            {
                ...
                LOG(INFO, "Partition with EPID 0x%x Found", ep_info[i].id);
                found = 1;
                break;
            }
        }
        if (!found)
        {
            ...
            LOG(INFO, "Partition with EPID 0x%x Not Found", ep_info[i].id);
        }
    }
...
void val_sync_ep_info_service(ffa_args_t *args)
{
    ...
    if (payload.fid == FFA_ERROR_32)
    {
        LOG(ERROR, "Direct Resp 32 failed err [0] %x [1] %x", payload.arg2, payload.arg3);
    ...
    LOG(DBG, "Updating val_endpoint_info[%x]:", i);
    LOG(DBG, "+------------+-----------------+-----------------+");
    LOG(DBG, "| Field      | New Value       | Old Value       |");
    LOG(DBG, "+------------+-----------------+-----------------+");
    LOG(DBG, "| name       | %-14s  | %-14s  |", ep_info.name, ep_table_ptr[i].name);
    val_memcpy(ep_table_ptr[i].name, ep_info.name, 5);
    LOG(DBG, "| id         | 0x%08x      | 0x%08x      |", ep_info.id, ep_table_ptr[i].id);
    ...
    LOG(DBG, "| tg0        | 0x%08x      | 0x%08x      |", ep_info.tg0, ep_table_ptr[i].tg0);
