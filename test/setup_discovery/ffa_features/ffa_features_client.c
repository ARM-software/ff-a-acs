...
static uint32_t ffa_feature_query(uint32_t fid, char *str)
{
    ...
    switch (fid)
    {
        ...
        case FFA_RXTX_MAP_32:
            ...
            {
                LOG(TEST, "\tRXTX_MAP alignment boundary is 4K size\n", 0, 0);
            }
            else if (data == FFA_RXTX_MAP_64K_SIZE)
            {
                LOG(TEST, "\tRXTX_MAP alignment boundary is 64K size\n", 0, 0);
            }
            else if (data == FFA_RXTX_MAP_16K_SIZE)
            {
                LOG(TEST, "\tRXTX_MAP alignment boundary is 16K size\n", 0, 0);
            }
            else
            {
        ...
        case FFA_MEM_DONATE_64:
            ...
            {
                LOG(TEST, "\tDynamic buffer supported\n", 0, 0);
            }
            else
            {
                LOG(TEST, "\tDynamic buffer not supported\n", 0, 0);
            }
