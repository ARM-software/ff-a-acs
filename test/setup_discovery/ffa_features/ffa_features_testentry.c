...
void ffa_features_testentry(uint32_t test_num)
{
    /* Execute test for EP combination: client=VM1, server=NONE */
    if (IS_TEST_FAIL(val_execute_test(test_num, VM1, NO_SERVER_EP)))
        return;

    /* Execute test for EP combination: client=VM2, server=NONE */
    if (IS_TEST_FAIL(val_execute_test(test_num, VM2, NO_SERVER_EP)))
        return;
