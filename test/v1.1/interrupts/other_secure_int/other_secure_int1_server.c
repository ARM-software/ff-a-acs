...
static uint32_t sp2_func(ffa_args_t args)
{
    ffa_args_t payload;
...
static uint32_t sp4_func(ffa_args_t args)
{
    ffa_args_t payload;
    uint32_t status = VAL_SUCCESS;
    ffa_endpoint_id_t sender = args.arg1 & 0xffff;
    ...
    LOG(DBG, "IRQ Status ptr %x", *(volatile uint32_t *)ptr);
...
uint32_t other_secure_int1_server(ffa_args_t args)
{
    ...
    {
        status = sp2_func(args);
    }
    else if (curr_ep_logical_id == SP4)
    {
        status = sp4_func(args);
    }
