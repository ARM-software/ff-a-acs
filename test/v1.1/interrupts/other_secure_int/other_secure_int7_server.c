...
static uint32_t sp2_entry(ffa_args_t args)
{
    ...
    LOG(DBG, "System Timer Handler Registered\n");
    ...
    LOG(DBG, "System Timer Interrupt Enabled\n");
    ...
    if (payload.fid != FFA_INTERRUPT_32)
    {
        LOG(ERROR, "Should have received FFA_INTERRUPT fid=0x%x, err 0x%x\n",
                  payload.fid, payload.arg2);
        status =  VAL_ERROR_POINT(18);
    }

    /* Disable system timer */
    val_sys_phy_timer_dis(false);
    LOG(DBG, "System Timer Interrupt Disabled\n");
...
static uint32_t sp4_entry(ffa_args_t args)
{
    ...
    /* Wait for system timer interrupt */
    sp_sleep(AP_REFCLK_TIMEOUT);
    LOG(DBG, "System Timer Interrupt Wait Complete\n");
...
uint32_t other_secure_int7_server(ffa_args_t args)
{
    uint32_t status = VAL_SUCCESS;
