...
uint32_t ffa_msg_send2_uuid_check_client(uint32_t test_run_data)
{
    ffa_args_t payload;
    ...
    partition_message_header->uuid[3] = 0xD;
#endif
    ...
    partition_message_header->uuid[3] = ep_info[server_logical_id].uuid[3];
#endif
    val_memset(&payload, 0, sizeof(ffa_args_t));
    payload.arg1 = ((uint32_t)sender << 16);
    ...
    LOG(DBG, "id_list_count %x expected_id_list_count %x receiver %x\n", id_list_count,
