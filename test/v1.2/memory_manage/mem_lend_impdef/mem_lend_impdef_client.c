...
static uint32_t mem_lend_impdef_helper(uint32_t test_run_data, uint32_t fid)
{
    ffa_args_t payload;
...
uint32_t mem_lend_impdef_client(uint32_t test_run_data)
{
    ...
    {
        status = mem_lend_impdef_helper(test_run_data, FFA_MEM_LEND_32);
    }
    else if (!status_64 && status_32)
    {
        status = mem_lend_impdef_helper(test_run_data, FFA_MEM_LEND_64);
    }
    else
    {
        status = mem_lend_impdef_helper(test_run_data, FFA_MEM_LEND_64);
