...
uint32_t mem_share_impdef_server(ffa_args_t args)
{
    ffa_args_t payload;
    ...
    /* Try Mem Retrieve with Invalid Impdef Values */
    mem_region_init.receivers[1].impdef.val[0] = 0;
    mem_region_init.receivers[1].impdef.val[1] = 0;
    ...
    {
        LOG(ERROR, "Mem retrieve request must fail for invalid impdef value err %x", payload.arg2);
        status =  VAL_ERROR_POINT(5);
        goto rxtx_unmap;
    }

#if (PLATFORM_FFA_V >= FFA_V_1_2)
    /* Try Mem Retrieve with valid Impdef Values */
