...
static uint32_t lend_multi_borrower_flag_check_helper(uint32_t test_run_data, uint32_t fid)
{
    ffa_args_t payload;
...
uint32_t lend_multi_borrower_flag_check_client(uint32_t test_run_data)
{
    ...
    {
        status = lend_multi_borrower_flag_check_helper(test_run_data, FFA_MEM_LEND_32);
    }
    else if (!status_64 && status_32)
    {
        status = lend_multi_borrower_flag_check_helper(test_run_data, FFA_MEM_LEND_64);
    }
    else
    {
        status = lend_multi_borrower_flag_check_helper(test_run_data, FFA_MEM_LEND_64);
