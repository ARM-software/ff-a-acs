...
uint32_t lend_ro_retrieve_rw_server(ffa_args_t args)
{
    ffa_args_t payload;
    ...
    /* Sender - FFA_DATA_ACCESS_RO, Receiver - FFA_DATA_ACCESS_RW.
     * Relayer must return DENIED if validation fails.
     * */
    ...
    {
        LOG(ERROR, "Mem retrieve request must failed with DENIED err %x", payload.arg2);
        status =  VAL_ERROR_POINT(5);
    }
    LOG(DBG, "Mem Retrieve Check for RO DATA ACCESS Complete");
