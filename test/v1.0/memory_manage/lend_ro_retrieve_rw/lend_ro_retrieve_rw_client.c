...
static uint32_t lend_ro_retrieve_rw_helper(uint32_t test_run_data, uint32_t fid)
{
    ffa_args_t payload;
    ...
    /* Check Read-write permission has precedence over Read-only permission */
...
uint32_t lend_ro_retrieve_rw_client(uint32_t test_run_data)
{
    ...
    {
        status = lend_ro_retrieve_rw_helper(test_run_data, FFA_MEM_LEND_32);
    }
    else if (!status_64 && status_32)
    {
        status = lend_ro_retrieve_rw_helper(test_run_data, FFA_MEM_LEND_64);
    }
    else
    {
        status = lend_ro_retrieve_rw_helper(test_run_data, FFA_MEM_LEND_64);
