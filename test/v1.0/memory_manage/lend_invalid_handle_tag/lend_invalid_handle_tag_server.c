...
static uint32_t mem_lend_invalid_handle_tag_check(ffa_memory_handle_t handle, uint32_t fid,
                uint32_t tag, void *tx_buf, ffa_endpoint_id_t sender, ffa_endpoint_id_t receiver)
{
    ...
    /* MEM_LEND: Relayer must ensure that the handle was allocated to the owner
     * specified in the sender endpoint ID field of the transaction descriptor
     */
    /* MEM_LEND: The Relayer must ensure the Tag value specified by the Receiver
    ...
    {
        LOG(ERROR, "Relayer must return error for invalid handle err %x\n",
                                payload.arg2);
    ...
    LOG(DBG, "Mem Retrieve with Invalid Handle Check Complete\n");
    ...
    LOG(DBG, "Mem Retrieve with Invalid Tag Check Complete\n");

err:
    return status;
}

uint32_t lend_invalid_handle_tag_server(ffa_args_t args)
{
    ffa_args_t payload;
    uint32_t status = VAL_SUCCESS;
    ...
    status = mem_lend_invalid_handle_tag_check(handle, fid, tag, mb.send, receiver, sender);
