...
static uint32_t ffa_mem_share_helper(uint32_t test_run_data, uint32_t fid)
{
    ...
    /* Corrupt tx buffer after MEM_SHARE. This should not have
     * any impact on memory sharing transcation as relayer must
     * copy the tx buffer of lender as part of MEM_SHARE operation.
     */
    val_memset(mb.send, 0xab, size);
...
uint32_t share_relinquish_input_checks_client(uint32_t test_run_data)
{
