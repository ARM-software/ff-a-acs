...
static uint32_t ffa_mem_lend_helper(uint32_t test_run_data, uint32_t fid)
{
    ...
    /* Set Zero memory before reclaim flag and check for zero content */
    payload.arg3 = 1;
    ...
    /* FFA_MEM_RECLAIM executes successfully, the state of the memory region for
     * the Owner must transition to Owner-EA.
     */
    constituents[0].address = val_mem_virt_to_phys((void *)pages);
    constituents[0].page_count = 1;
...
uint32_t reclaim_zero_flag_client(uint32_t test_run_data)
{
