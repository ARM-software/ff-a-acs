...
static uint32_t mem_share_device_attr_check(ffa_memory_handle_t handle, uint32_t fid,
...
uint32_t share_device_attr_server(ffa_args_t args)
{
    ffa_args_t payload;
    uint32_t status = VAL_SUCCESS;
    ...
    status = mem_share_device_attr_check(handle, fid, mb.send, receiver, sender, FFA_MEMORY_DEV_NGNRNE);
    if (status)
        goto rxtx_unmap;
