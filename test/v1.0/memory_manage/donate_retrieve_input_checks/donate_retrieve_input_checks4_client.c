...
static uint32_t donate_retrieve_input_checks4_helper(uint32_t test_run_data, uint32_t fid)
{
    ffa_args_t payload;
...
uint32_t donate_retrieve_input_checks4_client(uint32_t test_run_data)
{
    ...
    {
        status = donate_retrieve_input_checks4_helper(test_run_data, FFA_MEM_DONATE_32);
    }
    else if (!status_64 && status_32)
    {
        status = donate_retrieve_input_checks4_helper(test_run_data, FFA_MEM_DONATE_64);
    }
    else
    {
        status = donate_retrieve_input_checks4_helper(test_run_data, FFA_MEM_DONATE_64);
