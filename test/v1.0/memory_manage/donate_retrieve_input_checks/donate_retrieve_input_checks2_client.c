...
static uint32_t donate_retrieve_input_checks2_helper(uint32_t test_run_data, uint32_t fid)
{
    ffa_args_t payload;
    ...
    /* Check-1
     * Zero memory flag set to 0: Relayer must not zero the memory region contents
...
uint32_t donate_retrieve_input_checks2_client(uint32_t test_run_data)
{
    ...
    {
        status = donate_retrieve_input_checks2_helper(test_run_data, FFA_MEM_DONATE_32);
    }
    else if (!status_64 && status_32)
    {
        status = donate_retrieve_input_checks2_helper(test_run_data, FFA_MEM_DONATE_64);
    }
    else
    {
        status = donate_retrieve_input_checks2_helper(test_run_data, FFA_MEM_DONATE_64);
