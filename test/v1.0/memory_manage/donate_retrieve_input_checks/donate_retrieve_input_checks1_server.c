...
uint32_t donate_retrieve_input_checks1_server(ffa_args_t args)
{
    ffa_args_t payload;
    ...
    /* Relayer must return INVALID_PARAMETERS if the transaction type specified by the
      ...
      */
    mem_region_init.flags = (FFA_MEMORY_REGION_TRANSACTION_TYPE_LEND <<
    ...
    if (flags != FFA_MEMORY_REGION_TRANSACTION_TYPE_DONATE)
    {
