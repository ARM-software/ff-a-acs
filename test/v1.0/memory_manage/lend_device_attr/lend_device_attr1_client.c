...
static uint32_t mem_lend_device_attr_check(uint32_t test_run_data, uint32_t fid,
                void *tx_buf, enum ffa_memory_cacheability device_attr)
{
    ...
    {
        receiver_1 = val_get_endpoint_id(VM3);
    }
    else
    {
        receiver_1 = val_get_endpoint_id(SP3);
    }
    ...
    mem_region_init.receivers[0].receiver_permissions.receiver = receiver;
    ...
    mem_region_init.receivers[1].receiver_permissions.receiver = receiver_1;
    ...
    if (payload.fid == FFA_ERROR_32)
    {
        LOG(ERROR, "\tMEM_RECLAIM failed err %x\n", payload.arg2, 0);
...
static uint32_t ffa_mem_lend_handle_helper(uint32_t test_run_data, uint32_t fid)
{
    ...
    status = mem_lend_device_attr_check(test_run_data, fid, mb.send, FFA_MEMORY_DEV_NGNRNE);
    if (status)
        goto rxtx_unmap;

    status = mem_lend_device_attr_check(test_run_data, fid, mb.send, FFA_MEMORY_DEV_NGNRE);
    if (status)
        goto rxtx_unmap;

    status = mem_lend_device_attr_check(test_run_data, fid, mb.send, FFA_MEMORY_DEV_NGRE);
...
uint32_t lend_device_attr1_client(uint32_t test_run_data)
{
