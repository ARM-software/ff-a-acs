...
static uint32_t donate_state_machine_5_helper(uint32_t test_run_data, uint32_t fid)
{
    ffa_args_t payload;
    ...
    /* Sender has already share memory region to recipient and recipient_1.
     * Try to donate the same memory region to recipient again and check for error status code.
     */
    mem_region_init.receiver = recipient;
    ...
    if (payload.fid != FFA_ERROR_32)
    {
        LOG(ERROR, "\tMem_donate must return error for invalid memory state\n", 0, 0);
...
uint32_t donate_state_machine_5_client(uint32_t test_run_data)
{
    ...
    {
        status = donate_state_machine_5_helper(test_run_data, FFA_MEM_DONATE_32);
    }
    else if (!status_64 && status_32)
    {
        status = donate_state_machine_5_helper(test_run_data, FFA_MEM_DONATE_64);
    }
    else
    {
        status = donate_state_machine_5_helper(test_run_data, FFA_MEM_DONATE_64);
