...
static uint32_t lend_input_error_checks3_helper(uint32_t test_run_data, uint32_t fid)
{
    ffa_args_t payload;
    ...
    LOG(DBG, "Mem Lend Check for Invalid Instruction Access Complete\n");
...
uint32_t lend_input_error_checks3_client(uint32_t test_run_data)
{
    ...
    {
        status = lend_input_error_checks3_helper(test_run_data, FFA_MEM_LEND_32);
    }
    else if (!status_64 && status_32)
    {
        status = lend_input_error_checks3_helper(test_run_data, FFA_MEM_LEND_64);
    }
    else
    {
        status = lend_input_error_checks3_helper(test_run_data, FFA_MEM_LEND_64);
