...
static uint32_t lend_input_error_checks4_helper(uint32_t test_run_data, uint32_t fid)
{
    ffa_args_t payload;
    ...
    {
        LOG(ERROR, "Mem_lend request must return DENIED for Read only access to memory region %x",
                                                                             payload.arg2);
        status = VAL_ERROR_POINT(3);
        goto rxtx_unmap;
    }
    LOG(DBG, "Mem Lend Check for RO Memory Lend Complete");
...
uint32_t lend_input_error_checks4_client(uint32_t test_run_data)
{
    ...
    {
        status = lend_input_error_checks4_helper(test_run_data, FFA_MEM_LEND_32);
    }
    else if (!status_64 && status_32)
    {
        status = lend_input_error_checks4_helper(test_run_data, FFA_MEM_LEND_64);
    }
    else
    {
        status = lend_input_error_checks4_helper(test_run_data, FFA_MEM_LEND_64);
