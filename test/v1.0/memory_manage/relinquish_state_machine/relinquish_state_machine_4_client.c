...
static uint32_t ffa_mem_lend_helper(uint32_t test_run_data, uint32_t fid)
{
    ...
    /* Sender has lent memory region to recipient and recipient_1.
     * Sender reclaims exclusive access once all Borrowers have relinquished access.
     */
   ...
   /* Check that content of reclaimed memory is cleared
    * as one of the borrower sets the Zero memory after relinquish flag
    */
    for (i = 0; i < size; ++i)
    {
        if (pages[i] != 0)
...
uint32_t relinquish_state_machine_4_client(uint32_t test_run_data)
{
