...
static uint32_t ffa_mem_lend_helper(uint32_t test_run_data, uint32_t fid)
{
    ...
    /* Check that content of reclaimed memory is equal to zero as
     * Zero memory after relinquish flag is set */
    for (i = 0; i < size; ++i)
    {
        if (pages[i] != 0)
        {
...
uint32_t relinquish_state_machine_3_client(uint32_t test_run_data)
{
