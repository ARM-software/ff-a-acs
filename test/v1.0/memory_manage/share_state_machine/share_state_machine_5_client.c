...
static uint32_t ffa_mem_share_helper(uint32_t test_run_data, uint32_t fid)
{
     ...
     * Try to share the same memory region to recipient_1 and check for error status code.
     */
    ...
    LOG(DBG, "Mem Share after share check Complete\n");

   /* Sender has already shared memory region to recipient.
    * Try to share the some part of memory region to recipient_1
    * and check for error status code.
    */
    mem_region_init.receiver = recipient_1;
    /* Try to share lower 4k page */
...
uint32_t share_state_machine_5_client(uint32_t test_run_data)
{
