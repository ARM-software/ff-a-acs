...
uint32_t lend_retrieve_input_checks4_server(ffa_args_t args)
{
    ffa_args_t payload;
    ...
    /* instruction access is set to executable
     * and type is set to Device memory type
     * relayer must return DENIED error code
     */
    ...
    if (payload.fid != FFA_ERROR_32 || payload.arg2 != FFA_ERROR_DENIED)
    {
        ...
        if (payload.fid == FFA_MEM_RETRIEVE_RESP_32)
        {
            ...
            ffa_mem_relinquish_init((struct ffa_mem_relinquish *)mb.send, handle, 0, receiver, 0x1);
            val_memset(&payload, 0, sizeof(ffa_args_t));
    ...
    LOG(DBG, "Mem Retrieve Check for Invalid Instruction Access Complete");
