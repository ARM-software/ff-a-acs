...
uint32_t lend_retrieve_input_checks7_server(ffa_args_t args)
{
    ffa_args_t payload;
    ...
    /* Relayer must ensure that the identity of each Borrower specified
     * by the Receiver is the same as that specified by the Sender,
     * else must return INVALID_PARAMETERS in case of an error.
     */
    ...
    {
        LOG(ERROR, "FFA_MEM_RETRIEVE_REQ must return error for invalid borrower %x",
           payload.arg2);
        status =  VAL_ERROR_POINT(4);
        goto rxtx_unmap;
    }
    LOG(DBG, "Mem Retrieve Check for Invalid Borrower Complete");
