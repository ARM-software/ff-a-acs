...
static uint32_t lend_retrieve_input_checks9_helper(uint32_t test_run_data, uint32_t fid)
{
    ffa_args_t payload;
    ...
    /* Check memory read access and content of memory region
     * is equal to the data set by server
     */
    for (i = 0; i < size; ++i)
    {
        if (pages[i] != 1)
        {
            LOG(ERROR, "\tRegion data mismatch\n", 0, 0);
...
uint32_t lend_retrieve_input_checks9_client(uint32_t test_run_data)
{
    ...
    {
        status = lend_retrieve_input_checks9_helper(test_run_data, FFA_MEM_LEND_32);
    }
    else if (!status_64 && status_32)
    {
        status = lend_retrieve_input_checks9_helper(test_run_data, FFA_MEM_LEND_64);
    }
    else
    {
        status = lend_retrieve_input_checks9_helper(test_run_data, FFA_MEM_LEND_64);
