...
static uint32_t lend_retrieve_input_checks6_helper(uint32_t test_run_data, uint32_t fid)
{
    ffa_args_t payload;
...
uint32_t lend_retrieve_input_checks6_client(uint32_t test_run_data)
{
    ...
    {
        status = lend_retrieve_input_checks6_helper(test_run_data, FFA_MEM_LEND_32);
    }
    else if (!status_64 && status_32)
    {
        status = lend_retrieve_input_checks6_helper(test_run_data, FFA_MEM_LEND_64);
    }
    else
    {
        status = lend_retrieve_input_checks6_helper(test_run_data, FFA_MEM_LEND_64);
