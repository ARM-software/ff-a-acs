...
static uint32_t lend_retrieve_input_checks7_helper(uint32_t test_run_data, uint32_t fid)
{
    ffa_args_t payload;
    ...
    ffa_endpoint_id_t recipient_2 = val_get_endpoint_id(SP3);
    uint32_t test_run_data_2 = SET_SERVER_LOGIC_ID(test_run_data, SP3);
    mb_buf_t mb;
    uint8_t *pages = NULL;
...
uint32_t lend_retrieve_input_checks7_client(uint32_t test_run_data)
{
    ...
    {
        status = lend_retrieve_input_checks7_helper(test_run_data, FFA_MEM_LEND_32);
    }
    else if (!status_64 && status_32)
    {
        status = lend_retrieve_input_checks7_helper(test_run_data, FFA_MEM_LEND_64);
    }
    else
    {
        status = lend_retrieve_input_checks7_helper(test_run_data, FFA_MEM_LEND_64);
