...
static uint32_t lend_retrieve_input_checks3_helper(uint32_t test_run_data, uint32_t fid)
{
    ffa_args_t payload;
    ...
    /* Zero memory flag. - b'1: Relayer must zero the memory region contents */
    ...
    for (i = 0; i < size; ++i)
    {
        if (pages[i] != 0x0)
        {
            LOG(ERROR, "Region data mismatch after relinquish, page[%d]=%x\n", i, pages[i]);
...
uint32_t lend_retrieve_input_checks3_client(uint32_t test_run_data)
{
    ...
    {
        status = lend_retrieve_input_checks3_helper(test_run_data, FFA_MEM_LEND_32);
    }
    else if (!status_64 && status_32)
    {
        status = lend_retrieve_input_checks3_helper(test_run_data, FFA_MEM_LEND_64);
    }
    else
    {
        status = lend_retrieve_input_checks3_helper(test_run_data, FFA_MEM_LEND_64);
