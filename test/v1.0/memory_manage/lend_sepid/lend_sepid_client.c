...
static uint32_t ffa_mem_lend_helper(uint32_t test_run_data, uint32_t fid)
{
     ...
     * Try to lend the same memory region to recipient and
     * check owner is in Owner-NA state.
     */
    mem_region_init.receiver = recipient;
    val_ffa_memory_region_init(&mem_region_init, constituents, constituents_count);
    val_memset(&payload, 0, sizeof(ffa_args_t));
    ...
    LOG(DBG, "Mem Lend Check For Invalid Mem State Complete\n");
...
uint32_t lend_sepid_client(uint32_t test_run_data)
{
