...
static uint32_t share_rw_retrieve_ro_helper(uint32_t test_run_data, uint32_t fid)
{
    ffa_args_t payload;
...
uint32_t share_rw_retrieve_ro_client(uint32_t test_run_data)
{
    ...
    {
        status = share_rw_retrieve_ro_helper(test_run_data, FFA_MEM_SHARE_32);
    }
    else if (!status_64 && status_32)
    {
        status = share_rw_retrieve_ro_helper(test_run_data, FFA_MEM_SHARE_64);
    }
    else
    {
        status = share_rw_retrieve_ro_helper(test_run_data, FFA_MEM_SHARE_64);
