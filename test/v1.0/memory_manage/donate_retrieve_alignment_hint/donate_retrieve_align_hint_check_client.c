...
static uint32_t ffa_mem_donate_handle_helper(uint32_t test_run_data, uint32_t fid)
{
    ffa_args_t payload;
...
uint32_t donate_retrieve_align_hint_check_client(uint32_t test_run_data)
{
    ...
    {
        status = ffa_mem_donate_handle_helper(test_run_data, FFA_MEM_DONATE_32);
    }
    else if (!status_64 && status_32)
    {
        status = ffa_mem_donate_handle_helper(test_run_data, FFA_MEM_DONATE_64);
    }
    else
    {
        status = ffa_mem_donate_handle_helper(test_run_data, FFA_MEM_DONATE_64);
