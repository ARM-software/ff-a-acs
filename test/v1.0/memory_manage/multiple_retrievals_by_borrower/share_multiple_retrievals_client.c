...
static uint32_t share_multiple_retrievals_helper(uint32_t test_run_data, uint32_t fid)
{
    ffa_args_t payload;
...
uint32_t share_multiple_retrievals_client(uint32_t test_run_data)
{
    ...
    {
        status = share_multiple_retrievals_helper(test_run_data, FFA_MEM_SHARE_32);
    }
    else if (!status_64 && status_32)
    {
        status = share_multiple_retrievals_helper(test_run_data, FFA_MEM_SHARE_64);
    }
    else
    {
        status = share_multiple_retrievals_helper(test_run_data, FFA_MEM_SHARE_64);
