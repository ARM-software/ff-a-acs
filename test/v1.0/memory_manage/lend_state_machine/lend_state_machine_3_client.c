...
static uint32_t lend_state_machine_3_helper(uint32_t test_run_data, uint32_t fid)
{
    ffa_args_t payload;
     ...
     * Try to lend the same memory region to recipient and check for error status code.
     */
    mem_region_init.receiver = recipient;
    val_ffa_memory_region_init(&mem_region_init, constituents, constituents_count);
    val_memset(&payload, 0, sizeof(ffa_args_t));
...
uint32_t lend_state_machine_3_client(uint32_t test_run_data)
{
    ...
    {
        status = lend_state_machine_3_helper(test_run_data, FFA_MEM_LEND_32);
    }
    else if (!status_64 && status_32)
    {
        status = lend_state_machine_3_helper(test_run_data, FFA_MEM_LEND_64);
    }
    else
    {
        status = lend_state_machine_3_helper(test_run_data, FFA_MEM_LEND_64);
