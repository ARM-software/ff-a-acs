...
static uint32_t lend_state_machine_5_helper(uint32_t test_run_data, uint32_t fid)
{
    ffa_args_t payload;
    ...
    /* Sender has already shared memory region to recipient.
     * Try to lend the same memory region to recipient_1 and check for error status code.
     */
    ...
    if (payload.fid != FFA_ERROR_32)
    {
        LOG(ERROR, "Mem_lend must return error for invalid memory state\n");
        status = VAL_ERROR_POINT(6);
        goto rxtx_unmap;
    }
    LOG(DBG, "Mem Lend Check for Invalid Memory State Complete\n");
...
uint32_t lend_state_machine_5_client(uint32_t test_run_data)
{
    ...
    {
        status = lend_state_machine_5_helper(test_run_data, FFA_MEM_LEND_32);
    }
    else if (!status_64 && status_32)
    {
        status = lend_state_machine_5_helper(test_run_data, FFA_MEM_LEND_64);
    }
    else
    {
        status = lend_state_machine_5_helper(test_run_data, FFA_MEM_LEND_64);
