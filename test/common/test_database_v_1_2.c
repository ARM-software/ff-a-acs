...
#endif // ACS_FFA_UNVERIFIED
#endif // SUITE == interrupts
#endif // TARGET_LINUX
#endif // PLATFORM_FFA_V_ALL == 1

#if (PLATFORM_FFA_V_1_2 == 1 || PLATFORM_FFA_V_ALL == 1)
#if (SUITE == all || SUITE == setup_discovery)
    CLIENT_TEST_FN_ONLY(TESTSUITE_SETUP_DISCOVERY, ffa_version_negotiation),
    CLIENT_SERVER_TEST_FN(TESTSUITE_SETUP_DISCOVERY, ffa_msg_wait_rx_buff_rel),
    CLIENT_TEST_FN_ONLY(TESTSUITE_SETUP_DISCOVERY, ffa_partition_info_get_regs),
    CLIENT_TEST_FN_ONLY(TESTSUITE_SETUP_DISCOVERY, ffa_partition_info_get_lsp),
#ifdef ACS_FFA_UNVERIFIED
    CLIENT_TEST_FN_ONLY(TESTSUITE_SETUP_DISCOVERY, ffa_console_log),
#endif
#endif

#if (SUITE == all || SUITE == direct_messaging)
#ifdef ACS_FFA_UNVERIFIED
    CLIENT_SERVER_TEST_FN(TESTSUITE_DIRECT_MESSAGING, ffa_run_direct_req),
#endif
#endif
...
#ifdef ACS_FFA_UNVERIFIED
    CLIENT_SERVER_TEST_FN(TESTSUITE_MEMORY_MANAGE, lend_multi_borrower_flag_check),
#endif
#endif

#if (SUITE == all || SUITE == indirect_messaging)
#ifdef ACS_FFA_UNVERIFIED
    CLIENT_SERVER_TEST_FN(TESTSUITE_INDIRECT_MESSAGING, ffa_msg_send2_uuid_check),
#endif
#endif

#if (SUITE == all || SUITE == interrupts)
#if (PLATFORM_SP_EL == 1)
    CLIENT_SERVER_TEST_FN(TESTSUITE_INTERRUPTS, sp_el1_int_mask),
#ifdef ACS_FFA_UNVERIFIED
    CLIENT_SERVER_TEST_FN(TESTSUITE_INTERRUPTS, sp_yield_spmc_mode),
#endif
#endif
#endif
#if (SUITE == all || SUITE == direct_messaging)
