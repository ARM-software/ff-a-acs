...
static uint32_t ffa_mem_share_mmio_helper(uint32_t test_run_data, uint32_t fid)
{
    ffa_args_t payload;
...
uint32_t ffa_mem_share_mmio_client(uint32_t test_run_data)
{
    ...
    {
        status = ffa_mem_share_mmio_helper(test_run_data, FFA_MEM_SHARE_32);
    }
    else if (!status_64 && status_32)
    {
        status = ffa_mem_share_mmio_helper(test_run_data, FFA_MEM_SHARE_64);
    }
    else
    {
        status = ffa_mem_share_mmio_helper(test_run_data, FFA_MEM_SHARE_64);
