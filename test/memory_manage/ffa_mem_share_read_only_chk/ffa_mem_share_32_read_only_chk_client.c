...
static uint32_t ffa_mem_share_32_read_only_chk_client(uint32_t test_run_data)
{
    ffa_args_t payload;
    ...
    {
        LOG(TEST, "\tFFA_MEM_SHARE_32 not supported, skipping the check\n", 0, 0);
...
uint32_t ffa_mem_share_32_read_only_chk_vmsp_client(uint32_t test_run_data)
{
    return ffa_mem_share_32_read_only_chk_client(test_run_data);
}

uint32_t ffa_mem_share_32_read_only_chk_spsp_client(uint32_t test_run_data)
{
    return ffa_mem_share_32_read_only_chk_client(test_run_data);
}

uint32_t ffa_mem_share_32_read_only_chk_vmvm_client(uint32_t test_run_data)
{
    return ffa_mem_share_32_read_only_chk_client(test_run_data);
}
