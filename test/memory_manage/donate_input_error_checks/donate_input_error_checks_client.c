...
static uint32_t mem_donate_data_access_perm_check(void *tx_buf, ffa_endpoint_id_t sender, uint32_t fid)
{
    ffa_args_t payload;
    uint32_t status = VAL_SUCCESS;
    ...
    if ((payload.fid != FFA_ERROR_32) || (payload.arg2 != FFA_ERROR_INVALID_PARAMETERS))
    {
        LOG(ERROR, "\tFor MEM_DONATE, data access perm must be b'00\n", 0, 0);
...
static uint32_t mem_donate_mem_attribute_check(void *tx_buf, ffa_endpoint_id_t sender, uint32_t fid)
{
    ...
    if ((payload.fid != FFA_ERROR_32) || (payload.arg2 != FFA_ERROR_INVALID_PARAMETERS))
    {
        LOG(ERROR, "\tFor MEM_DONATE, memory type must be b'00\n", 0, 0);
