...
if [ ! -f "$BUILD_DIR/.config" ]; then
    make -C "$UBOOT_DIR" O="$BUILD_DIR" CROSS_COMPILE="$TOOLCHAIN" vexpress_fvp_defconfig -j$JOBS
    ...
    make -C "$UBOOT_DIR" O="$BUILD_DIR" CROSS_COMPILE="$TOOLCHAIN" olddefconfig -j$JOBS
fi

# Build U-Boot
make -C "$UBOOT_DIR" O="$BUILD_DIR" CROSS_COMPILE="$TOOLCHAIN" -j$JOBS
