...
REPO_URL="https://git.kernel.org/pub/scm/linux/kernel/git/sudeep.holla/linux.git"
BRANCH="for-bertrand/acs-dont-build"
...
if [ ! -d "$LINUX_DIR" ]; then
    ...
    git -C $LINUX_DIR checkout 5154b24f6ea7fb2df0e5b46b50a6fe627c9f6db6
fi
...
if [ ! -f "$BUILD_DIR/.config" ]; then
    ...
    make -C "$LINUX_DIR" -j$JOBS CROSS_COMPILE="$TOOLCHAIN" ARCH=arm64 O="$BUILD_DIR" olddefconfig
fi
make -C "$LINUX_DIR" -j$JOBS CROSS_COMPILE="$TOOLCHAIN" ARCH=arm64 O="$BUILD_DIR" Image
